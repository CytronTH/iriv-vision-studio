...
const WIDGET_TYPES = [
  ...
  { type: 'textFeed', label: '📝 Text / Log Feed', minW: 3, minH: 2 },
...
export default function LiveDashboard({ metadata, connected, projectId }) {
  ...
  return (
    <div className="animate-in fade-in duration-500 flex flex-col h-[calc(100vh-100px)]">
      ...
      {/* Dashboard Toolbar */}
      <div className="flex items-center justify-between mb-4 shrink-0 bg-gray-900 border border-gray-800 p-2 rounded-xl shadow-lg">
        <h2 className="text-xl font-bold ml-2 text-white">Live Operations Center</h2>
        <div className="flex-1 ml-4 text-xs font-mono text-green-500 overflow-hidden whitespace-nowrap overflow-ellipsis">
        ...
        <div className="flex gap-2">
          {isEditMode ? (
            ...
            <button 
              ...
              className="bg-gray-800 hover:bg-gray-700 text-gray-300 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors border border-gray-700"
            >
      ...
      <div className="flex flex-1 overflow-hidden relative">
          ...
          {layouts.lg.map(item => {
            ...
            return (
              <div key={item.i} className="relative group h-full w-full">
                ...
                {type === 'video' && <VideoWidget metadata={config?.stream_id ? metadata[config.stream_id] : metadata} projectId={projectId} config={config} />}
        ...
        {/* Edit Mode Sidebar */}
        {isEditMode && (
          <div className="w-64 bg-gray-900 border-l border-gray-800 p-4 shrink-0 flex flex-col gap-3 overflow-y-auto">
